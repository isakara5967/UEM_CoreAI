...
class EmotionCore:
    ...
    def _classify_emotion(self) -> str:
        """Simple PAD-based emotion classification (v1.5 - 0-1 arousal)"""
        # Arousal: 0-1 scale (0=calm, 1=excited)
        # Valence: -1 to 1 scale (-1=negative, 1=positive)
        
        high_arousal = self.arousal >= 0.45
        low_arousal = self.arousal < 0.35
        
        if self.valence > 0.3 and high_arousal:
            return 'joy'
        elif self.valence > 0.3 and low_arousal:
            return 'calm'
        elif self.valence > 0.25:
            return 'content'
        elif self.valence < -0.3 and high_arousal:
            ...
            return 'worried'
        elif abs(self.valence) < 0.2 and self.arousal < 0.4:
            return 'neutral'
        elif high_arousal:
            return 'excited'
        else:
    ...
    def evaluate(self, world_snapshot, state_vector) -> dict:
        """
        Appraisal hesaplamasını yapan resmi API (v1.5).
        
        Emotion v1.5 güncellemesi:
        - energy parametresi eklendi
        - event/symbol tabanlı valence boost
        - Daha geniş valence/arousal aralığı
        - Zengin emotion label çeşitliliği
        
        Args:
            world_snapshot: WorldSnapshot veya dict (danger_level, player_energy, symbols içerebilir)
        ...
        # Energy (NEW in v1.5)
        energy = 1.0
        if hasattr(world_snapshot, 'player_energy'):
            energy = world_snapshot.player_energy
        elif isinstance(world_snapshot, dict):
            energy = world_snapshot.get('player_energy', 1.0)
        
        # Symbols/Events (NEW in v1.5)
        symbols = []
        if hasattr(world_snapshot, 'symbols'):
            symbols = world_snapshot.symbols or []
        elif isinstance(world_snapshot, dict):
            symbols = world_snapshot.get('symbols', [])
        
        # === COMPUTE VALENCE (v1.5 - enhanced) ===
        
        # Base valence from danger and health
        valence = 0.0
        valence -= danger * 0.6  # Danger strongly negative
        valence += (health - 0.5) * 0.4  # Health deviation
        valence += (energy - 0.5) * 0.3  # Energy deviation (NEW)
        
        # Event-based valence boost (NEW in v1.5)
        positive_events = {'FOUND_FOOD', 'FOUND_WATER', 'FOUND_TREASURE', 'SAFE_ZONE', 'NPC_FRIENDLY'}
        negative_events = {'ENEMY_APPEARED', 'TOOK_DAMAGE', 'DANGER_HIGH', 'TRAPPED'}
        
        for symbol in symbols:
            if symbol in positive_events:
                valence += 0.2
            elif symbol in negative_events:
                valence -= 0.2
        
        valence = max(-1.0, min(1.0, valence))
        
        # === COMPUTE AROUSAL (v1.5 - enhanced) ===
        
        # Base arousal
        arousal = 0.3  # Lower baseline (was 0.5)
        arousal += danger * 0.5  # Danger increases arousal
        arousal += abs(valence) * 0.2  # Strong emotions increase arousal (NEW)
        
        # Novelty from symbols (NEW in v1.5)
        if symbols:
            arousal += 0.1  # Any event increases arousal slightly
        
        # Low energy decreases arousal
        if energy < 0.3:
            arousal -= 0.2
        ...
        dominance = 0.0
        dominance += (health - 0.5) * 0.4  # Health = power
        dominance += (energy - 0.5) * 0.3  # Energy = capability
        dominance -= danger * 0.4  # Danger = threat to control
